...
### 5. Wallet Context
**Owns:** Seller Wallets, Platform Wallet, Ledger Transactions, Payout Methods, Payout Requests, Payout Transactions, Commission Settlement  
**Maps to:** Wallet Service  
> Wallet is a shared abstraction with two owner types: `seller` (SellerWallet)
> and `platform` (PlatformWallet, holding accumulated commission revenue).
> Both share the same Ledger and Payout mechanics.
